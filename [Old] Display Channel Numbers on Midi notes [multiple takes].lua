-- This script might or might not work on your computer.
-- It is very primitive in its code. 
-- Function rarely return values. Most variables are global. The code is very procedural in its design.
-- Which makes it easy to design/troubleshoot, but hard for outsiders to analyse.
...
reaper.JS_LICE_SetFontColor( LICEFont,  0xFFFFFFF)  
...
function go() -- The order in which the takes are drawn determine its hierarchy.  Last one = top. First one = bottom.
   ...
   if _take_ then 
      getTakeNotes(_take_) -- The previous active take.
   end
   getTakeNotes( take ) -- last take is the active take
end 
        ...
        while stringPos < MIDIlen do 
           ...
           --  or  midi == 8
               ...
               if pitch<=topvisiblepitch and pitch> ME_BottomPitch then 
                  -- translate back to PPQpos relative to the ACTIVE take
                  ...
                  if take_ == _take_ then --previous active take.
                     reaper.JS_LICE_SetFontColor( LICEFont,   0xFFFFFF)  --gray
                  elseif take_== take then  --active take
                     reaper.JS_LICE_SetFontColor( LICEFont,   0xFFFFFF) -- white
                  else -- obscure takes
                    reaper.JS_LICE_SetFontColor( LICEFont,   0xA9A9A9) --gray
                  end
                  if flags&1==1 then  -- selected notes 
                    reaper.JS_LICE_SetFontColor( LICEFont,   0x000000)  --black
                  end
...
function main() 
   ...
   if hwnd then 
     ...
     hash_ = hash or "" 
     retval,tryhash  = pcall( function() -- Prevent script from crashing, when cutting,gluing,deleting midi items.
  ...
  ------------------------------------------------------ 
  if update then 
      readfromchunk() -- Ripped from Julian saders midi script.
      UnlinkBitmap() -- Removing channelnumbers
      go() -- Running trough all takes , analysing and drawing.
      update=false     
   end
   reaper.defer(main) 
end 
